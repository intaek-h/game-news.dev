...
kv.listenQueue(async (msg: unknown) => {
  if (MessageGuard.IsHotTopic(msg)) {
    ...
    // Set an expiration time (2 days) for the processed message record to prevent unlimited storage growth
    const expirationMs = 1000 * 60 * 60 * 24 * 2; // 2 days in milliseconds
